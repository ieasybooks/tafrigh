...
class Writer:
    ...
    def generate_txt(self, segments: list[dict[str, str | float]]) -> str:
        return '\n'.join(list(map(lambda segment: segment['text'].strip(), segments))) + '\n'

    def generate_srt(self, segments: list[dict[str, str | float]]) -> str:
        return ''.join(
            f"{i}\n"
    ...
    def generate_vtt(self, segments: list[dict[str, str | float]]) -> str:
        return 'WEBVTT\n\n' + ''.join(
