...
def process_local(
    path: Path, model: WhisperModel, config: Config
) -> List[List[Dict[str, Union[str, float]]]]:
    filtered_media_files: List[Path] = file_utils.filter_media_files(
        [path] if path.is_file() else path.iterdir()
    )
    ...
    for file in tqdm(files, desc="Local files"):
        ...
        elements_segments.append(
            writer.compact_segments(segments, config.output.min_words_per_segment)
        )
...
def process_url(
    ...
    for element in tqdm(url_data, desc="URL elements"):
        ...
        for segment in segments:
            segment[
                "url"
            ] = f"https://youtube.com/watch?v={element['id']}&t={int(segment['start'])}"
            segment["file_path"] = file_path
...
def write_output_sample(
    segments: List[Dict[str, Union[str, float]]], output: Config.Output
) -> None:
    ...
    with open(os.path.join(output.output_dir, "sample.csv"), "w") as fp:
        writer = csv.DictWriter(
            fp, fieldnames=["start", "end", "text", "url", "file_path"]
        )
        writer.writeheader()

        for segment in segments[: output.output_sample]:
            segment["start"] = time_utils.format_timestamp(
                segment["start"], include_hours=True, decimal_marker=","
            )
            segment["end"] = time_utils.format_timestamp(
                segment["end"], include_hours=True, decimal_marker=","
            )
